# Wrapper to keep compatibility with existing tests that import
# a top-level module filename. This file forwards imports to the
# package module at src/homeolabel/app.py.

import sys, os, logging

# Make sure src/ is on sys.path so `import homeolabel` works when running tests from repo root
ROOT = os.path.dirname(os.path.abspath(__file__))
SRC = os.path.join(ROOT, "src")
if SRC not in sys.path:
    sys.path.insert(0, SRC)

# Try to import package definitions; fall back to local implementations in this file if unavailable.
_HAD_HOMEOLABEL = False
try:
    from homeolabel.app import HomeoLabelApp as _ImportedHomeoLabelApp, \
        split_medicine_name as _imported_split, fit_lines_to_box as _imported_fit
    _HAD_HOMEOLABEL = True
except Exception as _e:
    logging.warning(f"homeolabel package not importable: {_e}. Falling back to local implementations in this file.")
    _ImportedHomeoLabelApp = None
    _imported_split = None
    _imported_fit = None

# If the package was importable, prefer its implementations
if _HAD_HOMEOLABEL:
    HomeoLabelApp = _ImportedHomeoLabelApp
    split_medicine_name = _imported_split
    fit_lines_to_box = _imported_fit

__all__ = ["HomeoLabelApp", "split_medicine_name", "fit_lines_to_box"]
...
class HomeoLabelApp(QtWidgets.QWidget):
    ...
    def apply_scaled_style(self):
        ...
        try:
            ...
            # Minimum sizes (ensure integer values for Qt)
            preview_min_w = max(0, int(round(self._ui['preview_min_width'] * self.scaling)))
            suggestion_min_w = max(0, int(round(self._ui['suggestion_min_width'] * self.scaling)))
            suggestion_min_h = max(0, int(round(self._ui['suggestion_min_height'] * self.scaling)))
            self.preview_frame.setMinimumWidth(preview_min_w)
            self.suggestion_table.setMinimumWidth(suggestion_min_w)
            self.suggestion_table.setMinimumHeight(suggestion_min_h)
    ...
    def add_new_medicine(self):
        # Ask for both Common and Latin names (Latin optional — will fallback to common if empty)
        common_name, ok1 = QtWidgets.QInputDialog.getText(self, "Add New Medicine - Common Name", "Enter common name:")
        if not ok1 or not common_name.strip():
            return
        latin_name, ok2 = QtWidgets.QInputDialog.getText(self, "Add New Medicine - Latin Name", "Enter Latin name (optional):")
        if not ok2:
            return
        latin = latin_name.strip() or common_name.strip()
        self.save_new_medicine(common_name.strip(), latin)
        # Default search to the common name after adding
        self.medicine_search.setText(common_name.strip())
    ...
    def save_new_medicine(self, med_name, latin_name=None):
        # Accept either: (med_name) or (common_name, latin_name)
        try:
            if latin_name is None and isinstance(med_name, (list, tuple)) and len(med_name) >= 2:
                common_name = med_name[0].strip()
                latin_name = med_name[1].strip()
            elif latin_name is not None:
                common_name = str(med_name).strip()
                latin_name = str(latin_name).strip()
            else:
            ...
            if not exists:
                ...
                try:
                    ...
                    logging.info(f"New medicine added: common='{common_name}', latin='{latin_name}'")
                except Exception as e:
                    logging.error(f"Failed to save remedies.xlsx: {e}")
                    QMessageBox.warning(self, "Save Failed", f"Could not save new medicine: {e}")
        except Exception:
            logging.exception("save_new_medicine failed")
...
# Attempt absolute import first (required). If that fails, load from likely filenames.
def _load_functions():
    try:
        # Absolute import as requested
        from homeo_label_printer_font_9_scaled import split_medicine_name, fit_lines_to_box
        return split_medicine_name, fit_lines_to_box
    except Exception:
        # Fallback: try to load module from common filenames in repo root
        candidates = [
            "homeo_label_printer_font9_responsive.py",
        ...
        for name in candidates:
            ...
            if os.path.exists(path):
                spec = importlib.util.spec_from_file_location("homeo_app_for_tests", path)
                mod = importlib.util.module_from_spec(spec)
                try:
                    spec.loader.exec_module(mod)
                except Exception as e:
                    # Surface the original import error with filename context
                    raise ImportError(f"Loading {path} failed: {e}") from e
                if hasattr(mod, "split_medicine_name") and hasattr(mod, "fit_lines_to_box"):
                    return mod.split_medicine_name, mod.fit_lines_to_box
        raise ImportError(
            "Could not import split_medicine_name and fit_lines_to_box. "
            "Ensure the target module is present and importable."
        )

split_medicine_name, fit_lines_to_box = _load_functions()

def test_split_medicine_name_simple():
    a, b = split_medicine_name("Arnica", "30C", max_chars=18)
...
def test_split_medicine_name_long():
    ...
    a, b = split_medicine_name(longname, "200C", max_chars=10)
    assert isinstance(a, str) and a != ""
    assert isinstance(b, str) and b != ""
    assert "200C" in b

def test_split_medicine_name_edge_cases():
    # empty name -> potency as second line
    a, b = split_medicine_name("", "6X", max_chars=10)
    assert a == ""
    assert "6X" in b

    # name exactly max_chars should go to first line, potency on second
    name = "ABCDEFGHIJ"  # 10 chars
    a, b = split_medicine_name(name, "12C", max_chars=10)
    assert a != ""
    assert "12C" in b

def test_fit_lines_to_box_basic(tmp_path):
    pdf_path = tmp_path / "tmp.pdf"
    c = canvas.Canvas(str(pdf_path), pagesize=(50 * mm, 30 * mm))
    lines = ["HELLO WORLD", "30C", "One Two Three Four Five"]
    out = fit_lines_to_box(lines, c, "Helvetica", 9, max_width_mm=44)
